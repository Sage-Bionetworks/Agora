...
export class RowChartComponent extends BaseChartComponent {
  ...
  updateXTicks(chart: dc.RowChart) {
    ...
    allTicks.each(function (this: any, i) {
  ...
  moveTextToElement(chart: dc.RowChart, el: HTMLElement, vSpacing = 0) {
    ...
    const tooltip = this.getTooltip(
      'x-axis',
      'chart-x-axis-tooltip row-chart-x-axis-tooltip'
    );
    ...
    const svgEl = chart
      .select('g.axis g.tick line.grid-line')
    ...
    if (svgEl) {
      ...
      svg.selectAll('text').each(function (d, i) {
        ...
        tickText
          ...
          .attr('transform', () => {
            return (
              'translate(' + ftransfx + ',' + (currentStep + vSpacing) + ')'
            );
          })
  ...
  override onResize() {
    ...
    this.resizeTimer = setTimeout(() => {
      self.chart
        .width(
          self.chartContainer.nativeElement.parentElement.offsetWidth - 100
        )
